...
Market Sentinel AI v2.11.3 is the operational alerting, paper-trading and local market simulation application. It includes real Yahoo Finance by default, replaceable Stooq and Alpha Vantage market-data adapters, deterministic demo data only when explicitly selected, provider-backed Yahoo instrument search, a public Binance order-book adapter, cached live RSS headline context with transparent sentiment scoring, quality-gated SQLite ingestion with visible cached-data fallback, causal OHLCV/technical features, causal multitimeframe alignment, an adaptive cost-aware supervised directional model with purged walk-forward metrics, trainable XGBoost and LightGBM models, cost-aware backtesting, persistent operational signals with entry/stop/target plans, per-product quantitative explanations for direction and trade levels, automatic prediction recording and later real-price scoring across watchlist horizons with weekend session awareness, short-horizon 10-minute and 2-hour predictions, complete horizon-by-horizon accuracy analytics with resettable evaluation history, paper risk analytics for estimated/realized PnL, VaR/CVaR, volatility, Sharpe, Sortino and drawdown, an interactive simulation account with configurable capital, LONG/SHORT positions, bounded leverage, margin, mark-to-market PnL, optional stop-loss and take-profit exits and detailed persisted trade history, a complete local control center with manual and periodic scans, a searchable persistent watchlist with directional color zones, interactive line and OHLC candlestick charts with selectable time windows, visible time axis and hover OHLC inspection, explicit single-direction decisions and uncertainty ranges, paper-trade history, health, drift and Astra usage panels, a self-diagnosing launcher that loads local configuration and installs UI dependencies, dry-run/webhook alerts, order-flow features, regime-aware ensembles, gated and budget-audited GPT-6 Astra contextual reasoning, persistent paper accounts and trades, one-minute default background market refresh, recovery backups, operational logs, a repository security gate and end-to-end release tests. It does not place live trades.
...
`dashboard-demo` creates a static report only. For the complete UI, use `Open-Market-Sentinel.bat`
or the `serve` command below.

For the visual workspace, after installing the API extra, double-click `Open-Market-Sentinel.bat`.
It starts the complete operational dashboard and opens [http://127.0.0.1:8765](http://127.0.0.1:8765). See
[docs/ui.md](docs/ui.md) for the watchlist, chart filters and the approximate future scenario
display.

The operational launcher uses Yahoo Finance by default and reads the local `.env` file. It does
not substitute demo candles when live data is unavailable; it reports that fresh data is missing.
